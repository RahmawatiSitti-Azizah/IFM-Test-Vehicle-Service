...
import com.mitrais.vehicleservicesystem.mapper.OperationMapper;
import com.mitrais.vehicleservicesystem.repository.OperationRepository;
...
class OperationServiceImplTest {
    ...
    @Test
    void testSearchByVehicle_withAllFilterNull_thenReturnEmptyData() throws Exception {
        when(repository.findAllBy(null, null, null, null, null)).thenReturn(List.of());
        List<Operation> result = serviceInTest.searchByVehicle(null, null, null, null, null, null);
        Assertions.assertTrue(result.isEmpty());
        verify(repository, times(1)).findAllBy(null, null, null, null, null);
    }

    @Test
    void testSearchByVehicle_withAllFilterFilled_thenReturnDataBasedOnSpecification() throws Exception {
        when(repository.findAllBy(any(), any(), any(), any(), any())).thenReturn(List.of(new Operation()));
        List<Operation> result = serviceInTest.searchByVehicle(null, null, null, null, null, null);
        Assertions.assertTrue(!result.isEmpty());
        verify(repository, times(1)).findAllBy(any(), any(), any(), any(), any());
    }

    @Test
    void testSearchByVehicle_withPartialFilterFilled_thenReturnDataBasedOnSpecification() throws Exception {
        when(repository.findAllBy(any(), any(), any(), any(), any())).thenReturn(List.of(new Operation()));
        List<Operation> result = serviceInTest.searchByVehicle(null, null, null, null, null, null);
        Assertions.assertTrue(!result.isEmpty());
        verify(repository, times(1)).findAllBy(any(), any(), any(), any(), any());
